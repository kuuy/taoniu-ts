...
export default function useBalances() {
  ...
  const { totalBalance, holdingBalance, liquidityBalance, stakingBalance, collectionBalance, membership, rewards } = useMemo(() => {
    ...
    if(streamToken) {
      streamBalance = toBigNumber(streamToken.balance ?? 0)

      if(streamToken.pools) {
        streamToken.pools.forEach(pool => {
          if(!pool.totalContribution || !pool.userContribution) return
          let contributionPercentage = toBigNumber(pool.userContribution).dividedBy(pool.totalContribution).times(100)
      ...
      liquidityBalance = tokenState.tokens.reduce((sum, current) => {
        ...
        if(current.pools) {
          current.pools.forEach(pool => {
            ...
            let contributionPercentage = toBigNumber(pool.userContribution).dividedBy(pool.totalContribution).times(100)
            let contributionShare = contributionPercentage.shiftedBy(-2)
            let zilAmount = toBigNumber(pool.baseReserve ?? 0).shiftedBy(-current.decimals).times(current.market_data.rate_zil).times(2).times(contributionShare)
    ...
    tokenState.tokens.forEach(token => {
      ...
      token.pools.forEach(pool => {
        if(!pool.userContribution || !pool.totalContribution || toBigNumber(pool.userContribution).isZero()) return

        token.rewards.filter(reward => reward.exchange_id === pool.dex).forEach(reward => {
          let contributionPercentage = toBigNumber(pool.userContribution!).dividedBy(pool.totalContribution ?? 0).times(100)
              let contributionShare = contributionPercentage.shiftedBy(-2)
