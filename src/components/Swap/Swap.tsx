...
import { Exchange } from '~/src/lib/exchange/exchange'
import { ZilSwap } from '~/src/lib/exchange/zilswap/zilswap'
import { STREAM_ADDRESS, ZIL_ADDRESS } from '~/src/constants'
import { BIG_ONE } from '~/src/utils/strings'
import { XCADDex } from '~/src/lib/exchange/xcaddex/xcaddex'
import getExchange from '~/src/lib/zilstream/getExchange'
...
const Swap = (props: Props) => {
  ...
  const { tokenInAddress, tokenOutAddress, slippage } = useSelector((state) => state.swap)
  ...
  const selectedCurrency: Currency = currencyState.currencies.find((currency: { code: any }) => currency.code === currencyState.selectedCurrency)!
  ...
  const { tokenIn, tokenOut } = useMemo(() => {
    return {
      tokenIn: tokenState.tokens.filter((t: { address: any }) => t.address === tokenInAddress)?.[0],
      tokenOut: tokenState.tokens.filter((t: { address: any }) => t.address === tokenOutAddress)?.[0]
    }
    ...
    <div>
      <div className="flex items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          {showFullscreen && <Link href="/swap"><Maximize size={16} /></Link>}
        </div>
      </div>
