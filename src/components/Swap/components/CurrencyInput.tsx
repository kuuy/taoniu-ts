...
import { cryptoFormat, currencyFormat, fieldFormat, numberFormat } from '~/src/utils/format'
      ...
      <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
        <div className="mt-2 flex-grow">
          <button
            ...
            onClick={() => onAmountChange(selectedToken?toBigNumber(selectedToken.balance??0).shiftedBy(-(selectedToken.decimals??4)):toBigNumber(0))}
          >
            Balance: {cryptoFormat(selectedToken?toBigNumber(selectedToken.balance??0).shiftedBy(-(selectedToken.decimals??4)).toNumber():0)}
          </button>
        </div>
        <div className="mt-2">
