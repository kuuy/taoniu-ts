...
import { BIG_ZERO } from '~/src/utils/strings'
...
function PortfolioPools() {
  ...
  let filteredTokens = tokenState.tokens.filter(token => {
    return token.pools && token.pools.filter(pool => pool.userContribution && !toBigNumber(pool.userContribution).isZero()).length > 0
  })

  let pools = filteredTokens.flatMap(token => token.pools ?? []).filter(pool => pool.userContribution && !toBigNumber(pool.userContribution).isZero())
