import AlertFormModal from '~/src/components/AlertFormModal'
...
import { labelForIndicator, labelForMetric, Metric } from '~/src/types/metric'
import { cryptoFormat, currencyFormat, numberFormat } from '~/src/utils/format'

const NotificationPermission = dynamic(
  () => import('~/src/components/NotifcationPermission'),
  { ssr: false }
)

function Alerts() {
  let tokenState = useSelector(state => state.token)
