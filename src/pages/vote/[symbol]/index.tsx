...
import LoadingProposals from '~/src/components/LoadingProposals'
import LoadingSpaceHeader from '~/src/components/LoadingSpaceHeader'
import TokenIcon from '~/src/components/TokenIcon'
import dayjs from 'dayjs'
import getGovernanceProposals from '~/src/lib/zilliqa/getGovernanceProposals'
            ...
            {proposals.map((proposal, index) => {
              return (
                <div key={index} className="py-4 border-b dark:border-gray-700 last:border-b-0">
                  <div><Link href={`/vote/${symbol}/${proposal.authorIpfsHash}`} className="font-medium">{proposal.msg.payload.name}</Link></div>
